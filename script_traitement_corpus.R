...
# b) Importation des données & backup


corpus_media <- read_delim("MIP Ovni/corpus_media/data_corpus_media/corpus_media.csv", 
                           delim = ";", escape_double = FALSE, locale = locale(encoding = "ISO-8859-1"), 
                           trim_ws = TRUE)

backup_data <- corpus_media

data_tibble <- as.tibble(corpus_media)


# Appropriation et nettoyage des données ----------------------------------


# On procède à une première vue globale des données

data_tibble %>% 
  summary()

# On remarque des erreurs au niveau de l'imputation des catégories de variables
...
# Pour analyser les données, on peut se poser plusieurs questions d'analyse afin d'en apprendre plus sur le jeu de données

# Questions : 

# 1) Quelles sont les chaînes qui ont parlé des OVNI en premier ?
# -> Qui a le plus de notices pour les années de diffusion les plus anciennes

# 2) Quelles sont les chaînes qui  parlent plus fréquemment des OVNI ?
# -> Autrement dit, quelles chaines ont le plus de notices ?
...
# TF1 a été la premiere chaine à parler des OVNI lors d'un débat consacré aux ovni en 1994
